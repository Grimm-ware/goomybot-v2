...
async function loadTest() {
	...
	var data = (await dbConnection.getFromCollectionByField("pokemon", {
			id: 1
		}));
...
async function addTierAndRegionToRaidPokemon() {
	...
	for (var i = 10001; i <= 10275; i++) {
		console.log(`adding fields to pokemon iteration: ${i}`);
		var pokemonData = (await dbConnection.getFromCollectionByField("raidPokemon", {
				id: i
			}))[0];
		console.log("getting species");
		var speciesData = (await dbConnection.getFromCollectionByField("species", {
		...
		var pokemonSpeciesData = (await dbConnection.getFromCollectionByField("pokemon", {
				id: speciesData.id
			}))[0];
...
async function loadItemPocket() {
	for (var i = 1; i <= 8; i++) {
		...
		if (data && data != 'Not Found') {
			...
			// You can handle the data here
			await dbConnection.upsertObject("itemPocket", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadItemCategory() {
	for (var i = 1; i <= 55; i++) {
		...
		if (data && data != 'Not Found') {
			...
			// You can handle the data here
			await dbConnection.upsertObject("itemCategory", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadMoves() {
	for (var i = 1; i <= 904; i++) {
		...
		if (data && data != 'Not Found') {
			...
			// You can handle the data here
			await dbConnection.upsertObject("moves", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadRaidPokemon() {
	for (var i = 10001; i <= 10275; i++) {
		...
		if (data && data != 'Not Found') {
			//console.log("Received data:", data);
			// You can handle the data here
			await dbConnection.upsertObject("raidPokemon", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadItems() {
	for (var i = 1; i <= 2109; i++) {
		...
		if (data && data != 'Not Found') {
			//console.log("Received data:", data);
			// You can handle the data here
			await dbConnection.upsertObject("items", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadBerries() {
	for (var i = 1; i <= 64; i++) {
		...
		if (data && data != 'Not Found') {
			//console.log("Received data:", data);
			// You can handle the data here
			await dbConnection.upsertObject("berries", data, {
				name: data.name
			});
		} else {
			console.log("Data was not found");
...
async function loadSpecies() {
	for (var i = 1; i <= 1017; i++) {
		...
		if (data && data != 'Not Found') {
			//console.log("Received data:", data);
			// You can handle the data here
			await dbConnection.upsertObject("species", data, {
				id: data.id
			});
		} else {
			console.log("Data was not found");
